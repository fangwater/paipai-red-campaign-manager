...
func TestImportMaituoCustomerDailyReplaysDuplicateToRepairUnassignedNotesIntegration(t *testing.T) {
	databaseURL := os.Getenv("TEST_DATABASE_URL")
	if databaseURL == "" {
		t.Skip("TEST_DATABASE_URL is not set")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	postgres, err := NewPostgres(ctx, databaseURL, "maituo-note-replay-integration")
	if err != nil {
		t.Fatal(err)
	}
	defer postgres.Close()
	...
	prefix := "note-replay-integration-" + time.Now().Format("20060102150405.000000000")
	fileName := prefix + ".xlsx"
	reportDate := time.Date(2098, 8, 6, 0, 0, 0, 0, time.UTC)
	defer func() {
		cleanup := context.Background()
		_, _ = postgres.pool.Exec(cleanup, "DELETE FROM maituo_customer_daily_notes WHERE note_id=$1", prefix+"-note")
		_, _ = postgres.pool.Exec(cleanup, "DELETE FROM maituo_customer_daily_subaccounts WHERE subaccount=$1", prefix+"-account")
		_, _ = postgres.pool.Exec(cleanup, "DELETE FROM maituo_customer_daily_import_runs WHERE file_name=$1", fileName)
	}()
	...
	snapshot := maituo.Snapshot{
		...
		Notes: []maituo.NoteDetail{{
			NoteID: prefix + "-note", NoteURL: "https://example.com", Category: "测评",
			Subaccount: prefix + "-account", CampaignName: "计划", Placement: "搜索",
			RowMetadata: maituo.RowMetadata{SourceRow: 2, ContentHash: prefix + "-note-hash"},
		}},
		Subaccounts: []maituo.SubaccountOverview{{
			SPU: prefix, Subaccount: prefix + "-account", Placement: "搜索",
			RowMetadata: maituo.RowMetadata{SourceRow: 2, ContentHash: prefix + "-subaccount-hash"},
		}},
	}
	first, err := postgres.ImportMaituoCustomerDaily(ctx, snapshot)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := postgres.pool.Exec(ctx, `
		UPDATE maituo_customer_daily_notes
		SET subaccount='',campaign_name=''
		WHERE report_date=$1 AND note_id=$2
	`, reportDate, prefix+"-note"); err != nil {
		t.Fatal(err)
	}

	replayed, err := postgres.ImportMaituoCustomerDaily(ctx, snapshot)
	if err != nil {
		t.Fatal(err)
	}
	if replayed.AlreadySaved || replayed.RunID == first.RunID {
		t.Fatalf("replay result = %+v, first = %+v", replayed, first)
	}
	exported, err := postgres.MaituoSubaccountSnapshot(ctx, prefix+"-account", reportDate)
	if err != nil || len(exported.Notes) != 1 || exported.Notes[0].Subaccount != prefix+"-account" {
		t.Fatalf("snapshot=%+v err=%v", exported, err)
	}

	duplicate, err := postgres.ImportMaituoCustomerDaily(ctx, snapshot)
	if err != nil {
		t.Fatal(err)
	}
	if !duplicate.AlreadySaved || duplicate.RunID != replayed.RunID {
		t.Fatalf("duplicate result = %+v, replayed = %+v", duplicate, replayed)
	}
}
