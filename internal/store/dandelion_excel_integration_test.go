...
func TestImportDandelionExcelIntegration(t *testing.T) {
	...
	if firstResult.ReportDate != "2099-01-01" || firstResult.Inserted != 1 || firstResult.Updated != 0 || firstResult.Unchanged != 0 || firstResult.Deleted != 0 {
	...
	saved, err := postgres.SavedDandelionExcelImports(ctx)
	if err != nil {
		t.Fatal(err)
	}
	foundLatest := false
	for _, item := range saved {
		if item.ReportDate != "2099-01-01" {
			continue
		}
		foundLatest = true
		if item.RunID != changedResult.RunID || item.FileName != changed.FileName {
			t.Fatalf("saved import = %+v", item)
		}
	}
	if !foundLatest {
		t.Fatalf("saved imports missing latest run: %+v", saved)
	}
	var title string
