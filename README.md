...
子账户层读取最新完整日报的“分子账户”表，统一使用 KPI 70；搜索使用回搜成本，信息流使用预计回流后成本，并返回消耗、回搜人数、诊断成本、回搜率、CPC、CTR、投放笔记数及最近 7 个自然日点位。点击子账户后先展示这些指标的数据总览，再展示计划诊断；成本列继续返回较上一自然日变化。计划层读取同日报的“笔记明细”表，搜索 KPI 为 30，信息流 KPI 为 70：
