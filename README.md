...
历史列表中的已保存日期可打开该 `report_date` 对应的笔记明细。明细保留笔记、子账户、计划名和场域维度；同一笔记在不同子账户或计划投放时保留各自独立的行。页面同时按 `分子账户` 工作表生成独立子账户日报目录；每份下载文件只包含该子账户对应的 `分子账户` 汇总数据和 `笔记明细`。
...
- `总览KPI`，业务键为 `报表日期 + 指标`
- `笔记明细`，业务键为 `report_date + note_id + subaccount + campaign_name + placement`（报表日期 + 笔记ID + 子账户 + 计划名 + 场域）
...
“笔记明细”包含工作簿提供的子账户和计划名，可直接用于按子账户拆分导出；`分子账户`行仍没有笔记 ID，不能根据日期、场域、SPU 或金额补全缺失的笔记归属。数据库视图 `maituo_customer_daily_search_user_overlap` 只保留分子账户与 SPU 汇总之间可直接计算的指标；为兼容旧读取方而保留的笔记重叠字段不再计算并返回 `NULL`。诊断接口中的 `cost` 和兼容字段 `original_cost` 均为分子账户日报的原始成本，`correction_coefficient` 返回 `null`，KPI 状态也只按该原始成本判断。
...
接口只返回可用报表日期，不再返回计划对比项。笔记明细保留原始账户和计划行，但此兼容接口不将日报消耗、回搜人数或成本重新分摊到聚光计划。
